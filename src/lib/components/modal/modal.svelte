<script>
  import { isModalOpen, isLeaderboardOpen } from '$lib/store/store.js';
  import {pArea, pFencedTiles, pIDs, pyodideRan, boardOccupiedTiles, country, playerID, pResultString, theEvent} from '$lib/store/pentominos.js';
  ...
  function openLeaderboardModal(){
    isLeaderboardOpen.set(true);
  }

  function pressedSubmit()
  {
    if (document.getElementById('playerID').value != null && document.getElementById('playerID').value != '')
    {
      $playerID = document.getElementById('playerID').value;
    }
    $country = document.getElementById('country').value;
    $theEvent = document.getElementById('theEvent').value;
    postResults($pIDs, $pArea, $playerID, $country, $pResultString, $theEvent);
    closeModal();
  ...
  $: playerIDph = ($pyodideRan) ? $playerID : 0;
...
<div class="modal-overlay" on:click={closeModal} on:keydown={closeModal}>
  <div class="modal-content" on:click|stopPropagation on:keydown|stopPropagation>
    ...
    <div class = "input-row theEvent">
        <div>
          <h1>🎟️ Event: </h1>
        </div>
        <div class= "eventInput">
          <select class="form-select" id="theEvent" name="theEvent">
            <option>None</option>
            <option value="ZW">Heidelberg Laureate Forum</option>
          </select>
        </div>
    </div>
    
    <button class="primary" on:click={() => pressedSubmit()}>submit score to leaderboard</button>
  </div>
  </div>
</div>
{/if}
